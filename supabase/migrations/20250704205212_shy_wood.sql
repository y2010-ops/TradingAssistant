/*
  # Add comprehensive Indian stock list
  
  1. New Data
    - Adds 100+ major Indian stocks from NSE/BSE
    - Includes stocks from all major sectors
    - Adds proper metadata (sector, name)
  
  2. Indexes
    - Ensures proper indexing for fast search
*/
...
VALUES
  ...
  -- Pharma & Healthcare
  ('DRREDDY', 'Dr. Reddy\'s Laboratories Limited', 'Pharmaceuticals'),
...
CREATE INDEX IF NOT EXISTS idx_stocks_symbol_search ON stocks USING gin (symbol gin_trgm_ops) WHERE symbol IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_stocks_name_search ON stocks USING gin (name gin_trgm_ops) WHERE name IS NOT NULL;
